...
class TestNode(Node):
    def __init__(self):
        ...
        # dumbbell tasks
        # self.dumbbell_order = ['4lb', '5lb', '6lb', '7lb']
        self.dumbbell_order = ['3lb', '4lb', '5lb', '6lb', '7lb']
        
        # predefined pick up and place down positions in the arm frame
        self.approach_pos = {
            '3lb': np.array([0.51, -0.28, 0.05]),
            '4lb': np.array([0.51, -0.17, 0.055]),
            '5lb': np.array([0.51, -0.06, 0.055]),
            '6lb': np.array([0.51, 0.05, 0.060]),
            '7lb': np.array([0.51, 0.17, 0.060])
        }
        # self.place_pos = {
        #     '4lb': np.array([0.1, -0.4, 0.05]),
        #     '5lb': np.array([0.1, -0.4, 0.07 + 0.055]),
        #     '6lb': np.array([0.1, -0.4, 0.07 + 0.072 + 0.065]),
        #     '7lb': np.array([0.1, -0.4, 0.07 + 0.072 + 0.075 + 0.07])
        # }
        self.place_pos = {
            '3lb': np.array([0.1, -0.4, 0.04]),
            '4lb': np.array([0.1, -0.4, 0.06 + 0.05]),
            '5lb': np.array([0.1, -0.4, 0.06 + 0.07 + 0.055]),
            '6lb': np.array([0.1, -0.4, 0.06 + 0.07 + 0.072 + 0.07]),
            '7lb': np.array([0.1, -0.4, 0.06 + 0.07 + 0.072 + 0.075 + 0.07])
        }
        # self.approach_pos = {
        #     '3lb': np.array([0.51, -0.17, 0.045]),
        #     '4lb': np.array([0.51, -0.06, 0.05]),
        #     '5lb': np.array([0.51, 0.05, 0.055]),
        #     '6lb': np.array([0.51, 0.17, 0.055])
        # }
        # self.place_pos = {
        #     '3lb': np.array([0.1, -0.4, 0.055]),
        #     '4lb': np.array([0.1, -0.4, 0.055 + 0.055]),
        #     '5lb': np.array([0.1, -0.4, 0.055 + 0.07 + 0.065]),
        #     '6lb': np.array([0.1, -0.4, 0.055 + 0.07 + 0.072 + 0.07])
        # }
    ...
    def pick_up_loop(self):
        ...
        if self.current_task_index == 0:
            if self.trajectory_remaining_time == 0:
                approach_pos = self.approach_pos[self.dumbbell_order[self.dumbbell_id]]
                self.approach_q1 = self.IK(approach_pos + self.offset_pos)
            ...
            if self.trajectory_remaining_time == 0:
                approach_pos = self.approach_pos[self.dumbbell_order[self.dumbbell_id]]
                self.approach_q2 = self.IK(approach_pos)
                ...
                self.goto(self.approach_q2, start_time_1, 2.0)
                self.traj_pub.publish(self.traj_msg)
                self.traj_id += 1
                time.sleep(0.01)
                ...
                self.goto(self.approach_q1, start_time_3, 2.0, self.approach_q2)
                self.traj_pub.publish(self.traj_msg)
                self.traj_id += 1
                time.sleep(0.01)
    ...
    def place_down_loop(self):
        ...
        if self.current_task_index <= 0:
            ...
            if self.trajectory_remaining_time == 0:
                ...
                self.place_q1 = self.IK(place_pos + self.offset_pos)
                print(self.place_q1)
            ...
            if self.trajectory_remaining_time == 0:
                place_pos = self.place_pos[self.dumbbell_order[self.dumbbell_id]]
                self.place_q2 = self.IK(place_pos)
                print(self.place_q2)
    ...
    def endeffector_identification(self, exciting_id, traj_id):
        ...
        if exciting_id > 0:
            # print("Filtering trajectory: ", traj_id)
            # data = np.loadtxt('log/torque_output_' + str(traj_id) + '.txt')
            ...
            # np.savetxt('log/torque_output_' + str(traj_id) + '_filtered.txt', data_filtered, fmt='%.12f')
            
            # add the filtered trajectory file name to the list
            # self.sysid_solver.add_trajectory_file('log/torque_output_' + str(traj_id) + '_filtered.txt')
